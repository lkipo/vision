...
class zoomWindow:
    ...
    def show(self, image):
        ...
        putText(scaled,'{}x'.format(2**self.S))
...
class Video:
    ...
    def write(self, frame, key=0, keypressed=ord('v')):
        if key==keypressed:
            ...
            print('video: {}'.format(msg[self.ON]))
        ...
        if self.ON:
            if not self.Open:
                ...
                self.Open = True
                print('recording {} video {} with resolution {}x{} at {}fps'.format(self.codec,fname,w,h,fps))
            self.out.write(frame)
            self.nf += 1
    def release(self):
        if self.Open:
            self.out.release()
            print('{} frames written to video'.format(self.nf))
